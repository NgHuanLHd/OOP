...
public class ProjectileManager {
	...
	private void importImgs() {
		...
		proj_imgs[3] = LoadSave.getImageFromFile("AnotherTower/BulletCannon.png");
	...
	public void newProjectile(Tower t, Enemy e) {
		if (e.getEnemyType() == helpz.Constants.Enemies.BAT && t.getTowerType() != ANOTHER) {
		...
		if (type == ARROW || type == CUSTOM_PROJ) {
	...
	private boolean isProjHittingEnemy(Projectile p) {
		for (Enemy e : playing.getEnemyManger().getEnemies()) {
			...
			// Kiểm tra nếu kẻ địch là BAT và chỉ chấp nhận đạn của ANOTHER
            if (e.getEnemyType() == helpz.Constants.Enemies.BAT) {
                if (p.getProjectileType() != CUSTOM_PROJ) {
                    continue; // Bỏ qua nếu không phải đạn của ANOTHER
                }
            }
	...
	public void draw(Graphics g) {
		...
		for (Projectile p : projectiles)
			if (p.isActive()) {
				if (p.getProjectileType() == ARROW ||  p.getProjectileType() == CUSTOM_PROJ) {
